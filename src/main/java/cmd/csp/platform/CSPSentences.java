...
public class CSPSentences {
	...
	public void SetPos(int zerobasedpos) {
		if(zerobasedpos>=0 && this.count>0) {
			...
			current = new CSPSentence(store.getJsonObject(currentPos));
		}else {
			if(this.count>0) {
				currentPos = 0;
				current = new CSPSentence(store.getJsonObject(currentPos));
	...
	public CSPSentences(JsonObject jsonObj) {
		setFromJson( jsonObj);
	...
	public void setFromJson(JsonObject jsonObj) {
		...
		if(jsonObj.containsKey("sentence")) this.store=jsonObj.getJsonArray("sentence");
		if(store!=null) {
	...
	public void addToJson(JsonObject jcon) {
		jcon.put("sentences", this.getAsJsonObject());
	}
	public void UpdateLength() {
		if(store.size()>0) {
			...
			for(int i=0; i < store.size();i++) {
				...
				CSPSentence item = new CSPSentence(jo);
				item.UpdateLength();
